...
export class WebSocketManager {
  ...
  initialize(wss: WebSocketServer, redis: RedisClient) {
    this.wss = wss;
    this.redis = redis;
  ...
  private async sendSessionState(ws: AuthenticatedWebSocket) {
    if (!ws.sessionId || !this.redis) return;
  ...
  sendMessage(sessionId: string, message: WebSocketMessage) {
  ...
  sendNextDialRequest(sessionId: string) {
    this.sendMessage(sessionId, {
  ...
  sendStateUpdate(sessionId: string, readyState: string) {
    this.sendMessage(sessionId, {
      type: 'STATE',
      payload: { readyState }
    });
